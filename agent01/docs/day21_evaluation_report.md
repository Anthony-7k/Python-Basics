...
截至 2026-08-28，本地 Python 3.11.9 环境的 160 项测试、Python 编译检查、Day19 Agent 路由门槛、Day17 离线检索门槛和已保存真实响应的回答质量重放均通过。`v1.0.0` 发布提交对应的 GitHub Actions 运行 `33164825390` 已成功；后续文档提交也要求 CI 通过。
...
2026-08-31 从远端 `main@0991533` 进行了独立 clean-clone 复验：无缓存镜像构建、四服务健康、Alembic head、非 root/只读根文件系统、本地认证与知识库 CRUD、MySQL/Chroma/Upload 隔离卷 `down/up` 持久化、日志脱敏和无网络容器内 160 项回归均通过。由于未获得把指定样例发送到具体第三方模型服务的单独授权，clean-clone 没有重复外部模型调用；2026-08-28 已完成的 A/B 两轮真实模型链路仍作为该项证据。
...
## 6. Clean-clone 复验与发布验收矩阵

| Clean-clone 检查 | 结果 | 证据 |
| --- | --- | --- |
| 来源与构建 | 通过 | 远端 `main@0991533`，Git 工作区干净，`--no-cache` 构建 `agent01:day21-cleanclone` |
| 运行与安全 | 通过 | 四服务 healthy；三个 HTTP 检查和 UI 页面 200；Alembic head；UID 10001；只读根文件系统 |
| 隔离与持久化 | 通过 | 独立端口 `18000/18501`、独立三卷；`down/up` 后知识库记录和 Chroma/Upload 哨兵保留 |
| 日志 | 通过 | 7026 个日志字符中已知敏感值、密钥名/Bearer、Fatal/Traceback 命中均为 0 |
| 离线回归 | 通过 | 160/160；路由 12/12；Recall@3 72.5%；MRR 70%；NoAnswer 80%；回答质量 1.98/2 |
| 外部模型边界 | 未重复调用 | clean-clone 未重新外发样例；沿用 2026-08-28 A/B 两轮真实模型链路证据 |

| 级别 | 项目 | 状态 | 备注 |
| --- | --- | --- | --- |
| P0 | Docker Compose 配置展开 | 通过 | GitHub Actions 已验证 |
| P0 | 独立 clean-clone 可复现启动 | 通过 | 2026-08-31，隔离镜像、端口和三类数据卷 |
...
| P1 | README、架构、评测、演示与面试材料一致 | 通过 | 事实与当前仓库状态一致；视频按用户指示暂缓 |
| P1 | GitHub Actions 发布提交运行通过 | 通过 | `v1.0.0` 对应运行 `33164825390`；后续文档提交也要求 CI 通过 |
