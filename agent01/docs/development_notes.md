...
# Day 19 - Agent 工具调用：检索、总结与对比

- 新增 `ToolCall`、`ToolResult`、工具错误和 Agent 轨迹 Pydantic 契约。
- 建立只包含检索、单文档总结和双文档对比的白名单注册表；各工具参数使用
  独立 schema，并强制等于请求允许的 `knowledge_base_id`。
- 使用确定性路由区分普通问答、总结、对比、缺少参数和危险操作，不引入开放式
  ReAct 循环；最大步骤、单工具超时、总预算和证据上限均可配置。
- 三个工具在执行时重新使用认证用户和服务层授权；对比工具分别校验左右文档。
- 新增 `/api/v1/agent/run`，使用独立 `agent` scope 和 Chat 相同的次数限制。
- 工具轨迹使用结构化字段白名单，只记录调用元数据和安全摘要。
- 修正旧 RAG 检索日志仍包含原始/改写问题的偏差，改为请求 ID、知识库、
  检索模式、耗时和来源数量等结构化元数据。
- 固定 12 个路由样例，并加入白名单、schema、跨库、超时、步数、日志和恶意
  证据测试。
- 修复 Windows 反斜杠文件名在 Linux 上未正确取 basename 的跨平台回归。
